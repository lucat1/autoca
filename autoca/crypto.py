...
def issue_cert(
    ...
    """
    The common name is the first string in names
    """
    assert(not host or ca_cn is not None)
    SN = names[0]
    issuer = str(ca_cn) if host else SN
    ...
    builder = _add_SANs(builder, names)
    # TODO: change the way duration is computed, do modulo stuff for reproducibility
    builder = builder.not_valid_before(datetime.datetime.today() - ONE_DAY)
    builder = builder.not_valid_after(datetime.datetime.today() + (ONE_DAY * maximum_days))
    builder = builder.serial_number(x509.random_serial_number())
    builder = builder.public_key(public_key)
    # NOTE: maybe we need to have a type option, that is either (host, intermediate, ca)
    # as we need to set false to ca here in the case of intermediate, but we
    # don't want it to be self-signed, so it's something in between a host CSR and a CA cert
    builder = builder.add_extension(
        x509.BasicConstraints(ca=not host, path_length=None), critical=True
    )
